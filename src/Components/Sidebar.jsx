...
function Sidebar() {
    ...
    return ( 
        <div ref={sideRef} className={`w-[80%] sidebar h-screen absolute z-[2000] ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-slate-100'} transition  ${isSidebar ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className={`flex flex-col gap-6 border-4 border-x-0 ${isDarkMode ? 'border-y-slate-600': 'border-y-slate-100'} py-4 px-4`}>
            ...
            <div className="flex flex-col gap-8 py-6 text-lg font-[500] px-6">
                ...
                <div className="mode flex gap-4 justify-start">
                    Dark mode
                    <div onClick={handleMode} className={`w-14 h-7  rounded-2xl transition delay-300 duration-300 ${isDarkMode ? 'bg-green-500' : 'bg-slate-300'}`}>
                        <div className={`w-7 h-7 rounded-full bg-slate-100 transition delay-300 duration-300 ${isDarkMode ? 'translate-x-7' : 'translate-x-0'}`}>
                        </div>
                    </div>
                </div>
            </div>
